...
**`list` is model-then-render (#13, the #59 seam).** `collect_views` does the two inference passes once and returns `Vec<WorktreeView>` — the row model (branch, absolute path, bare/current/detached, full `lineage`, `children`+`children_overflow`, `Status`, `ahead`/`behind`). Three renderers consume it: `render_human` (compact/verbose — the *only* place `*`/`▸`/relative paths/`[N]` compaction lives), `render_porcelain` (git-style `key value` records), and `--json` via `serde_json` (`WorktreeView` derives `Serialize`; `#[serde(skip)]` on the human-only `label`/`abs`). Machine output is lossless by construction — decoration exists solely in `render_human`. `ListFormat` (Human/Porcelain/Json) selects; `main.rs` maps `--porcelain`/`--json` (clap `conflicts_with_all`, also vs `--verbose`) to it. Lineage is exposed at chain len ≥ 2 (a base branch still reports its parent), while the human display gate stays len ≥ 3 or has-children. (The full serializable per-row model that unifies these with the lineage — the #59 seam #13 will serialize — is deferred to #13, which knows its output shape; #30 renders inline since the loop already holds the other fields.) An empty branch at its base's commit shows no lineage until it gets a commit (transient, self-healing). Rebase/re-parenting is out of scope; navigation is `up`/`down`/`top`/`bottom` (see below).
