...
**`list` stack tree** (#37): worktrees render as a tree, parent-first. A branch's parent is **recorded at `add` time** in git config (`branch.<name>.tonic-parent`, namespaced, shared across worktrees via the common config — no bespoke file; `stored_parent`), so intent survives even an empty branch that sits at its base's commit. For branches tonic didn't create, fall back to **inferring** the parent from the commit graph (`infer_parent_label`: nearest strict-ancestor branch by `rev-list --count`, else `main`/`master`). A stored parent is only used if that branch is itself a listed worktree (else the child would orphan → inference). Rendering order is the pure, cycle-guarded `stack_order`; detached/bare worktrees have no branch to place and render flat. Rebase/re-parenting is explicitly out of scope.
