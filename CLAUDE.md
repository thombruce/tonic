...
**`list` stack lineage** (`lineage`): the list stays flat (worktrees are sibling dirs, not a tree). A row is annotated `root → … → branch` when the branch is stacked on a non-default branch, and a base row shows the worktrees stacked on it (`↳ child, child`). Lineage is *inferred* from the commit graph — one bounded `rev-list --first-parent --boundary <default>..<branch>` per worktree (cost = stack height, not branch count or history depth; `--boundary` + skipping default's tip commit handle a base whose tip has drifted into main), matched against a `sha → branch` tip map (`branch_tips`) — **no stored state**. `list` is two passes: infer every worktree's chain, derive the worktree-children map from immediate parents, then render. An empty branch at its base's commit shows no lineage until it gets a commit (transient, self-healing). Rebase/re-parenting is out of scope; navigation (`up`/`down`) is a separate concern (#43).
