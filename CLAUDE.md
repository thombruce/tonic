...
**`list` stack lineage** (`lineage`/`infer_parent`): the list stays flat (worktrees are sibling dirs, not a tree). A row is annotated `root → … → branch` only when the branch is stacked on a non-default branch. Parent is *inferred* from the commit graph (nearest strict ancestor by `rev-list --count`, else `main`/`master`) over **all local branches** (so the lineage can name ancestors without worktrees) — **no stored state**. An empty branch sitting at its base's commit has no strict ancestor there, so it shows no lineage until it gets a commit (transient, self-healing). Rebase/re-parenting is out of scope; navigation (`up`/`down`) is a separate concern (#43).
